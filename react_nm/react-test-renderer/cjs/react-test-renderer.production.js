...
const scheduleCallback$3 = Scheduler$1.unstable_scheduleCallback;
const cancelCallback$1 = Scheduler$1.unstable_cancelCallback;
...
const NormalPriority$1 = Scheduler$1.unstable_NormalPriority;
const IdlePriority = Scheduler$1.unstable_IdlePriority; // this doesn't actually exist on the scheduler, but it *does*
...
const clearSuspenseBoundary = shim$1;
const clearSuspenseBoundaryFromContainer = shim$1;
...
function pushHostContext(fiber) {
  ...
  const context = requiredContext(contextStackCursor.current);
  const nextContext = getChildHostContext(); // Don't push this Fiber's context unless it's unique.
...
const SuspenseyCommitException = new Error('Suspense Exception: This is not a real error, and should not leak into ' + "userspace. If you're seeing this, it's likely a bug in React."); // This is a noop thenable that we use to trigger a fallback in throwException.
...
function create(element, options) {
  ...
  let root = createContainer(container, isConcurrent ? ConcurrentRoot : LegacyRoot, null, isStrictMode, concurrentUpdatesByDefault, '', onRecoverableError, null);
