...
function aa(a){for(var b=a[1],c=[],d=0;d<b.length;d++){var e=b[d],g=w.get(e);if(void 0===g){g=__turbopack_load__(e);c.push(g);var f=w.set.bind(w,e,null);g.then(f,y);w.set(e,g)}else null!==g&&c.push(g)}return 4===a.length?0===c.length?x(a[0]):Promise.all(c).then(function(){return x(a[0])}):0<c.length?Promise.all(c):null}
